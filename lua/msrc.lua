...
local config = {
    ...
    i2c1 = {selected = 5, list = {"NONE", "BMP280", "MS5611", "BMP180", ""}, elements = 4},
...
local function readConfig()
    if readConfigState ~= state["MAINTENANCE_OFF"] then
        ...
        if primId == 0x32 and dataId == dataIdSensor then
            ...
            elseif bit32.extract(value, 0, 8) == 0xF2 and readConfigState == state["PACKET_1"] then
            ...
            elseif bit32.extract(value, 0, 8) == 0xF3 and readConfigState == state["PACKET_2"] then
            ...
            elseif bit32.extract(value, 0, 8) == 0xF4 and readConfigState == state["PACKET_3"] then
        ...
        -- timeout
        if getTime() - tsReadConfig > 600 then
...
local function refreshHorus()
    ...
    lcd.drawText(40, 62, "Voltage", 0)
    lcd.drawText(170, 62, config.voltage1.list[config.voltage1.selected], getFlags(2))

    lcd.drawText(40, 81, "Ntc", 0)
    lcd.drawText(170, 81, config.ntc1.list[config.ntc1.selected], getFlags(3))

    lcd.drawText(40, 100, "Current", 0)
    lcd.drawText(170, 100, config.current.list[config.current.selected], getFlags(4))
    lcd.drawText(290, 100, "AirSpeed", 0)
    lcd.drawText(410, 100, config.airspeed.list[config.airspeed.selected], getFlags(5))

    lcd.drawText(40, 119, "PWM out", 0)
    lcd.drawText(170, 119, config.pwm.list[config.pwm.selected], getFlags(6))
    lcd.drawText(290, 119, "GPS", 0)
    lcd.drawText(410, 119, config.gps.list[config.gps.selected], getFlags(7))

    lcd.drawText(40, 138, "Rate RPM", 0)
    lcd.drawText(170, 138, (config.refreshRpm.selected - 1) * 100, getFlags(8))
    lcd.drawText(290, 138, "Rate Volt", 0)
    lcd.drawText(410, 138, (config.refreshVolt.selected - 1) * 100, getFlags(9))

    lcd.drawText(40, 157, "Rate Curr", 0)
    lcd.drawText(170, 157, (config.refreshCurr.selected - 1) * 100, getFlags(10))
    lcd.drawText(290, 157, "Rate Temp", 0)
    lcd.drawText(410, 157, (config.refreshTemp.selected - 1) * 100, getFlags(11))

    lcd.drawText(40, 176, "Avg RPM", 0)
    lcd.drawText(170, 176, config.queueRpm.selected, getFlags(12))
    lcd.drawText(290, 176, "Avg Volt", 0)
    lcd.drawText(410, 176, config.queueVolt.selected, getFlags(13))

    lcd.drawText(40, 195, "Avg Curr", 0)
    lcd.drawText(170, 195, config.queueCurr.selected, getFlags(14))
    lcd.drawText(290, 195, "Avg Temp", 0)
    lcd.drawText(410, 195, config.queueTemp.selected, getFlags(15))

    lcd.drawText(40, 214, "I2C", 0)
    lcd.drawText(170, 214, config.i2c1.list[config.i2c1.selected], getFlags(16))
    lcd.drawText(290, 214, "Address", 0)
    lcd.drawText(410, 214, config.i2c1Address.selected - 1, getFlags(17))

    if readConfigState ~= state["MAINTENANCE_OFF"] then
        lcd.drawText(180, 155, "Connecting...", INVERS)
    end
    if sendConfigState ~= state["MAINTENANCE_OFF"] then
        lcd.drawText(200, 253, "UPDATING", getFlags(18))
    else
        lcd.drawText(200, 253, "UPDATE", getFlags(18))
    end
end
...
local function refreshTaranis()
    ...
    lcd.drawText(1, 25 - scroll * 8, "Voltage", SMLSIZE)
    lcd.drawText(44, 25 - scroll * 8, config.voltage1.list[config.voltage1.selected], SMLSIZE + getFlags(2))
    lcd.drawText(64, 25 - scroll * 8, "Ntc", SMLSIZE)
    lcd.drawText(108, 25 - scroll * 8, config.ntc1.list[config.ntc1.selected], SMLSIZE + getFlags(3))

    lcd.drawText(1, 33 - scroll * 8, "Current", SMLSIZE)
    lcd.drawText(44, 33 - scroll * 8, config.current.list[config.current.selected], SMLSIZE + getFlags(4))
    lcd.drawText(64, 33 - scroll * 8, "AirSpeed", SMLSIZE)
    lcd.drawText(108, 33 - scroll * 8, config.airspeed.list[config.airspeed.selected], SMLSIZE + getFlags(5))

    lcd.drawText(1, 41 - scroll * 8, "PWM out", SMLSIZE)
    lcd.drawText(44, 41 - scroll * 8, config.pwm.list[config.pwm.selected], SMLSIZE + getFlags(6))
    lcd.drawText(64, 41 - scroll * 8, "GPS", SMLSIZE)
    lcd.drawText(108, 41 - scroll * 8, config.gps.list[config.gps.selected], SMLSIZE + getFlags(7))
    ...
    lcd.drawText(1, 81 - scroll * 8, "I2C", SMLSIZE)
    ...
    if sendConfigState ~= state["MAINTENANCE_OFF"] then
        lcd.drawText(1, 89 - scroll * 8, "UPDATING", SMLSIZE + getFlags(18))
    else
        lcd.drawText(1, 89 - scroll * 8, "UPDATE", SMLSIZE + getFlags(18))
    end
