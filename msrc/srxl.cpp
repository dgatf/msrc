...
void Srxl::send()
{
    ...
    SMARTPORT_SRXL_FRSKY_SERIAL.write(buffer, 21);
#ifdef DEBUG
...
#else
    if (SMARTPORT_SRXL_FRSKY_SERIAL.available())
    {
        static uint32_t timestamp = 0;
        uint8_t buffer[SRXL_FRAMELEN];
        static bool mute = false;
        uint8_t lenght = SMARTPORT_SRXL_FRSKY_SERIAL.readBytes(buffer, SRXL_FRAMELEN);
        ...
        DEBUG_SERIAL.println();
            ...
            else
            {
                timestamp = millis();
                delay(1);
                while (SMARTPORT_SRXL_FRSKY_SERIAL.available() && millis() - timestamp < 8000)
                {
                    char buf[21];
                    uint8_t lenght = SMARTPORT_SRXL_FRSKY_SERIAL.readBytes(buf, 21);
                    if (lenght == 21 && !SMARTPORT_SRXL_FRSKY_SERIAL.available())
                        delay(1);
                }
                send();
                mute = true;
