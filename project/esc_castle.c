...
static void castle_link_handler(castle_link_telemetry_t packet)
{
    ...
    *parameter.temperature = packet.temperature;
    if (debug)
    {
        printf("\nCastle (%u) < Volt(V): %.2f Ripple volt(V): %.2f Curr(A): %.2f Thr: %.0f Out: %.0f Rpm: %.0f Bec volt(V): %.2f Bec curr(A): %.2f Temp(C): %.0f", uxTaskGetStackHighWaterMark(NULL), packet.voltage, packet.ripple_voltage, packet.current, packet.thr, packet.output, packet.rpm, packet.voltage_bec, packet.current_bec, packet.temperature);
