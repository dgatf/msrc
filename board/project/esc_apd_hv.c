    ...
    if (uart1_available() == ESC_APD_HV_PACKET_LENGHT)
    {
        ...
        if (get_crc16(data) == (((uint16_t)data[19] << 8) | data[18]))
        {
            ...
            *parameter->cell_voltage = *parameter->voltage / *parameter->cell_count;
            if (debug)
                printf("\nApd HV (%u) < Rpm: %.0f Volt: %0.2f Curr: %.2f Temp: %.0f Cons: %.0f CellV: %.2f", uxTaskGetStackHighWaterMark(NULL), *parameter->rpm, *parameter->voltage, *parameter->current, *parameter->temperature, *parameter->consumption, *parameter->cell_voltage);
        }
    }
}
