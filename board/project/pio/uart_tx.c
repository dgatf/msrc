...
uint uart_tx_init(PIO pio, uint pin, uint baudrate, uint data_bits, uint stop_bits, uint parity) {
    ...
    pio_->instr_mem[offset_] = pio_encode_set(pio_x, parity == UART_PARITY_NONE ? data_bits : data_bits + 1);
    pio_->instr_mem[offset_] = pio_encode_set(pio_pins, 1) | pio_encode_delay(16 * stop_bits - 3);
