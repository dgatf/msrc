...
static void process(rx_protocol_t rx_protocol) {
    ...
    if (rx_protocol == RX_SMARTPORT) {
        ...
        uint8_t data[] = {0x7E, 0x19, 0x00, 0x43, 0x03, 0xDE, 0xD0, 0xD0, 0x97, 0x3E, 0x56, 0x4C, 0x9C, 0x15,
                          0xAC, 0x48, 0xDF, 0xC4, 0x93, 0x07, 0x3E, 0xF0, 0x41, 0x7B, 0xE2, 0x00, 0x0F, 0xE2,
                          0x7E, 0x7E, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF6, 0x7E};
        //uint8_t data[] = {0x7E, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF6, 0x7E};
        for (uint8_t i = 0; i < sizeof(data); i++) {
            xQueueSendToBack(uart_queue_handle, &data[i], 0);
        }
    }
        ...
        uint8_t data[] = {0x19, 0xFF, 0x02, 0x07, 0x1F, 0x2B, 0xA2, 0x07, 0x3E, 0xF0, 0x81, 0xAF, 0x7C, 0xDC, 0x03, 0x1F, 0xF8, 0xC0, 0x07, 0x3E, 0xF0, 0x09, 0x7C, 0x15, 0x00, 0x64, 0x38,
                          0x08, 0x8E, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xEF};  // sensor id 15 = 0x8E
        for (uint8_t i = 0; i < sizeof(data); i++) {
            xQueueSendToBack(uart_queue_handle, &data[i], 0);
        }
    }
