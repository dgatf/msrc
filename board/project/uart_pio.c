...
void uart_pio_write(uint8_t c) {
    uart_tx_write(c);
}

void uart_pio_write_bytes(uint8_t *data, uint8_t lenght) {
    uart_tx_write_bytes(data, lenght);
}

uint8_t uart_pio_available(void) { return uxQueueMessagesWaiting(context.uart_rx_pio_queue_handle); }

uint8_t uart_pio_tx_available(void) { return UART_PIO_BUFFER_SIZE - uxQueueMessagesWaiting(context.uart_rx_pio_queue_handle); }
