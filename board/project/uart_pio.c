...
void uart_pio_begin(uint baudrate, uint gpio_rx, uint timeout, PIO pio, uint irq)
{
    ...
    uart_pio_queue_handle = xQueueCreate(UART_PIO_BUFFER_SIZE, sizeof(uint8_t));
}
...
static void uart_pio_handler(uint8_t data)
{
    ...
    if (uart_pio_timeout_alarm_id)
        alarm_pool_cancel_alarm(uart_alarm_pool, uart_pio_timeout_alarm_id);
...
{
    uint8_t value = 0;
    xQueueReceive(uart_pio_queue_handle, &value, 0);
    return value;
}
...
void uart_pio_read_bytes(uint8_t *data, uint8_t lenght)
{
    ...
    {
        xQueueReceive(uart_pio_queue_handle, data + i, 0);
    }
}

uint8_t uart_pio_available()
{
    return uxQueueMessagesWaiting(uart_pio_queue_handle);
}

uint uart_pio_get_time_elapsed()
{
    return time_us_32() - uart_pio_timestamp;
}
